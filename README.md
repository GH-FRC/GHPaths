...
> 状态：**Phase 0（单机链路验证）准备中**。可行性论证见 [GHPaths_可行性研究报告.md](GHPaths_可行性研究报告.md)（v1.3），其 41 条论断的独立核查记录见 [核查勘误](GHPaths_可行性研究报告_核查勘误_2026-08-18.md)。
...
**演出操作注意**：
- **单主控时钟**：同一时刻只允许一个演控台广播演出时钟。两个控制台同时发布会在机器人端触发时钟跳变防护（fault 闩锁,需 stop→arm 复位）——这是安全设计。开发时关掉旧标签页再开新的。
- **标签页可见性**：时钟广播与 uiPing 走 Worker 节拍（不受标签页定时器节流影响），但完全隐藏的标签页仍可能被 Chrome 冻结页面 JS（Worker 节拍到了页面侧被扣押）→ 时钟断流 → 机器人跳变防护触发。排练时保持演控台窗口可见；根治靠 Tauri 原生打包（ADR-0007,原生窗口无标签节流）。
...
Phase 0 单机链路验证 → Phase 1 双机 PoC → Phase 2 演控 MVP → Phase 3 多机路径编辑器 → Phase 4 全编制联排（详见报告第十一节）。
