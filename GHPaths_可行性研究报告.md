...
| **GHPaths 应用分发（.dmg 双击安装）** | ✅（免费方案） | ✅ 同左 | 已定稿免费分发（ADR-0007 v2）：浏览器下载 = 每台 Mac 首装一次性"仍要打开"；终端/Homebrew 渠道零弹窗；升级走应用内更新免再批准。详见 §7.5 |
...
**结论（v1.5 定稿，依用户决策：完全免费）**：

- **主渠道 = GitHub Releases 浏览器下载 .dmg**。免费上限即 PathPlanner/Choreo 同款：每台 Mac 首装一次性"仍要打开" + 密码（约 30 秒，全程无终端），之后该机永久正常；首次运行再点一次"允许本地网络"（应用内弹窗）。
- **升级免再批准**：浏览器每下载新版本都会重新触发一次批准，故默认升级路径为**应用内更新**（Tauri updater；应用进程下载的文件无隔离属性，不触发 Gatekeeper）——把"每版本烦一次"压成"每台机器终身一次"。列入打包后实测项。
- **并行渠道（零弹窗）**：终端一行安装脚本（curl 下载无隔离属性）与 Homebrew（自维护 tap 的 cask），面向会终端的队友与内部装机。
- **Releases 页面规范（用户要求）**：每个 release 固定包含——① 安装包（dmg 等）；② 浏览器安装图文教程（含"仍要打开"三步引导）；③ 终端与 Homebrew 安装命令。做成 release 模板，列入发版 checklist（ADR-0007）。
- **工程红线**：保留 ad-hoc 签名（Tauri 默认，禁止构建剥离——签名损坏会把"仍要打开"变成只剩 xattr 终端路的"已损坏"死胡同）；multi-DS 等子进程 sidecar 收进 .app；Info.plist 声明 NSLocalNetworkUsageDescription；dmg 背景图与下载页放同款安装引导。
- 若未来需要对外大规模分发的零警告体验，再重启开发者账号评估（$99/年或教育减免；仅记录，不在计划内）。
...
| 免费分发首装需人工"仍要打开"（现场/他人装机场景有沟通成本） | 中低 | ADR-0007 v2：Releases 页三步图文引导 + dmg 背景同款引导；内部装机走终端/Homebrew 零弹窗渠道；应用内更新免版本级再批准；未来可选升级开发者账号 |
...
> v1.4→v1.5 修订：Tauri 打包里程碑（首次正式发版）按免费分发方案验收（ADR-0007 v2）：ad-hoc 签名保留、应用内更新可用、dmg 背景与 Releases 页含安装包 + 浏览器/终端/Homebrew 三渠道安装教程。
...
- **安装体验最终是什么样（v1.4→v1.5，已定稿免费方案）？** 浏览器下载安装包为主渠道：每台 Mac 首装一次"仍要打开" + 密码（约 30 秒，无终端），之后含升级在内永久无感（升级走应用内更新）；终端/Homebrew 渠道零弹窗；Releases 页固定附安装包与三种安装教程。免费的物理上限即此，PathPlanner/Choreo 同款。
